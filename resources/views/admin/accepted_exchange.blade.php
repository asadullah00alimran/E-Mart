...
<div class="modal fade" id="staticBackdrop{{$exchange->exc_id}}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  ...
  {{-- Cancel --}}
<div class="modal fade" id="staticBackdropp{{$exchange->exc_id}}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="staticBackdropLabel">Exchange Id #{{$exchange->exc_id+100}}</h5>
        ...
        <div class="modal-body">
          <form action="{{ route('cancel_exchange_admin1') }}" method="POST">
            @csrf
            <input type="hidden" name="id" value="{{$exchange->exc_id}}">
            <div class="form-group">
                <label for=""><strong>Cancel Reason</strong></label>
                <textarea name="cancel_reason" class="form-control border-success" rows="8" required></textarea>
            </div>
            <div class="form-group">
                <button type="submit" class="btn btn-danger float-right ml-2">Calcel</button>
  ...
  {{-- end cancel --}}
                        <td class="text-center">
                            ...
                            <a data-toggle="modal" data-target="#staticBackdropp{{$exchange->exc_id}}" style="font-size: 11px;" class="btn btn-danger">Cancel</a>
                        </td>
